...
#   $tmpOut/$tmpErr 的路径与下游读取逻辑完全不变。
$tmpOut = Join-Path $env:TEMP ("agent_start_out_" + $PID + ".txt")
$tmpErr = Join-Path $env:TEMP ("agent_start_err_" + $PID + ".txt")
$childArgs = @('-ExecutionPolicy', 'Bypass', '-NoProfile', '-File', ('"' + $script:bin + '"'), '-Action', 'start')
$child = Start-ProcessClean -FilePath 'powershell.exe' -ArgumentList $childArgs -RedirectStandardOutput $tmpOut -RedirectStandardError $tmpErr -WaitSeconds 60
